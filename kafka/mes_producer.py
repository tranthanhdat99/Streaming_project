...
def create_kafka_consumer():
    ...
    """
    consumer = Consumer(config.SOURCE_KAFKA)
    consumer.subscribe([config.TOPICS['source']])
    consumer_logger.info(f"Subscribed to source topic '{config.TOPICS['source']}'")
    return consumer
...
def create_kafka_producer():
    ...
    """
    producer = Producer(config.DEST_KAFKA)
    producer_logger.info(f"Initialized producer for topic '{config.TOPICS['intermediate']}'")
    return producer
...
def replicate_messages():
    ...
    try:
        while True:
            ...
            producer.produce(
                config.TOPICS['intermediate'], msg.value(), callback=delivery_callback
            )
            producer.poll(0)
